...
- La tabla tiene trigger `CZ_MI.ARMISOE_BR`: en **INSERT** asigna `USUARIO_CREA`, `FECHA_CREA`, `USUARIO_MODIFICA` y `FECHA_MODIFICA`; en **UPDATE** actualiza `USUARIO_MODIFICA` y `FECHA_MODIFICA`. Para cargas iniciales, incluir en el INSERT solo las columnas de negocio (`ID`, `NOMBRE`) y dejar que el trigger complete auditoría.
...
**Descripción:** Catálogo de estados de solicitud según el flujo operativo acordado.
...
| ------------------ | ------------- | ------- | --------------------------------------------------------------------- |
| `ID`               | VARCHAR2(1)   | No      | PK. Código de catálogo de un carácter; clave del estado de solicitud. |
| `NOMBRE`           | VARCHAR2(100) | No      | Nombre o descripción del estado (pantallas e informes).               |
...
**Constraint:** `ARMISOE_PK` PRIMARY KEY (`ID`).

---

## Filas a insertar

Los códigos `ID` son un solo carácter (`VARCHAR2(1)`): secuencia **`A` … `I`** (abecedario en orden alfabético), asignada en el orden del flujo operativo de la tabla siguiente.

Solo se especifican datos de negocio; auditoría la completa el trigger `ARMISOE_BR`.
...
-- Repetir para ID 'B'..'I' y los nombres de la tabla anterior; luego:
COMMIT;
```
